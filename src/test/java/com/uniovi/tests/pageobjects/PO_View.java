...
public class PO_View {
	 ...
	 * @param type: 
	 * @param text:
	 * @param tiempoEspera: tiempo de espera deseado, o -1 para omitir.
	 * @param elementoParaClick: elemento sobre el que hacer click, o -1 para omitir.
	 * @return Se retornará la lista de elementos resultantes de la búsqueda.
	 */
	static public List<WebElement> checkElement(WebDriver driver, String type, String text, int tiempoEspera, int elementoParaClick) {
		List<WebElement> elementos = SeleniumUtils.EsperaCargaPagina(driver, type, text, getTimeout());
		
		if(tiempoEspera != -1) {
			SeleniumUtils.esperarSegundos(driver, tiempoEspera);
		}
		
		if(elementoParaClick != -1) {
			elementos.get(elementoParaClick).click();
		}
